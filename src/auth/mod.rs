...
mod processes_test {
    ...
    fn incorrect_token() {
        let mock_request = TestRequest::with_header(
            "blogger-token", "test").to_srv_request();
        match process_token(&mock_request) {
            Err(message) => assert_eq!(message, "Could not decode"),
            _ => panic!("Incorrect token should not be decodeable")
        }
    }
    ...
    fn correct_token() {
        ...
        let mock_request = TestRequest::with_header(
            "blogger-token", encoded_token).to_srv_request();
        match process_token(&mock_request) {
            Ok(token) => assert_eq!(token, "passed"),
            _ => panic!("Correct token present in header should
                             return passed")
        }
    }
